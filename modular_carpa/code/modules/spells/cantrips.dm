...
/obj/effect/proc_holder/spell/self/acidsplash5e/cast(mob/user = usr)
	...
	target.visible_message(span_warning("[target] lanza una burbuja cáustica!"), span_notice("¡Lanzaste una burbuja cáustica!"))
	. = ..()
...
/datum/status_effect/buff/acidsplash5e/on_apply()
	...
	owner.visible_message(span_warning("[owner] esta cubierto de ácido!"), span_danger("Estoy cubierto en acido!"))
	owner.emote("scream")
...
/obj/effect/proc_holder/spell/self/bladeward5e/cast(mob/user = usr)
	...
	user.visible_message("<span class='info'>[user] traza un sigilo de proteccion en el aire.</span>", "<span class='notice'>Trazo un sigilo de proteccion en el aire.</span>")
...
/datum/status_effect/buff/boomingblade5e/proc/boom()
	...
	owner.visible_message(span_warning("Un estruendo atronador emana de [owner]!"), span_danger("¡Un estruendo atronador emana de ti!"))
...
/obj/effect/proc_holder/spell/invoked/chilltouch5e/cast(list/targets, mob/living/user)
	if(isliving(targets[1]))
		...
		if(user.zone_selected == BODY_ZONE_CHEST && !target.cmode && target.client.prefs.sexable) //must be out of combat mode and have erp panel allowed for this prompt to appear
			target.visible_message(span_warning("¡Una mano esqueletica agarra la [bodypart] de [target]!"), span_danger("Una mano esquelética agarra mi [bodypart]!"))
...
/obj/item/chilltouch5e/on_embed(obj/item/bodypart/bodypart)
	to_chat(bodypart.owner, "<span class='warning'>Mano adherida a la [bodypart] de [bodypart.owner]!</span>")
	if(bodypart.owner)
		host = bodypart.owner
		START_PROCESSING(SSobj, src)
	...
	if(hand_proc == 1)
		switch(bodypart.name)
			if(BODY_ZONE_HEAD) //choke
				to_chat(host, "<span class='warning'>[host] esta siendo ahorcado por una mano esqueletica!</span>")
				playsound(get_turf(host), pick('sound/combat/shove.ogg'), 100, FALSE, -1)
				host.emote("choke")

				target.adjustOxyLoss(oxy_drain*mult*2)
			// esto es erp por lo tanto lo desactive
			...
			// 		target.adjustBruteLoss(oxy_drain*mult*3)
				...
				to_chat(host, "<span class='danger'>[bodypart] de [host] esta siendo retorcido por una mano esqueletica!</span>")
...
/obj/effect/proc_holder/spell/invoked/decompose5e/cast(list/targets, mob/living/user)
	if(!isliving(targets[1]))
		...
		if(iscarbon(target))
			...
			for(var/obj/item/bodypart/bodypart as anything in stinky.bodyparts)
				if(bodypart.rotted || bodypart.skeletonized)
					...
					break
		if(has_rot)
			to_chat(user, span_warning("Ya descompuesto."))
			return FALSE
		...
		if(HAS_TRAIT(target, TRAIT_ROTMAN))
			target.visible_message(span_notice("El cuerpo de [target] se descompone!"), span_green("Me siento descompuesto!"))
		else
			target.visible_message(span_warning("El cuerpo de [target] falla en pudrirse!"), span_warning("No me siento diferente.."))
		return TRUE
	return FALSE
...
/obj/effect/proc_holder/spell/targeted/encodethoughts5e/cast(list/targets, mob/user)
	...
	for(var/mob/living/carbon/C in targets)
		if(!C)
			return
		var/message = stripped_input(user, "Que pensamiento deseas tejer en la mente de [C]?")
		if(!message)
			return
		to_chat(C, "Tu mente piensa para si misma: </span><font color=#7246ff>\"[message]\"</font>")
		to_chat(user, "Toco las cuerdas de la mente de [C]")
		log_game("[key_name(user)] le envio un pensamiento a [key_name(C)] con el siguiente contenido: [message]")
		return TRUE
	to_chat(user, span_warning("No pude encontrar una mente para tejer aqui."))
	revert_cast()
...
/obj/effect/proc_holder/spell/targeted/guidance5e/cast(list/targets, mob/living/user)
	for(var/mob/living/carbon/C in targets)
		...
		to_chat(C, span_info("Estas iluminado por la luz guia de [user]."))
		C.visible_message(span_info("[C] esta siendo iluminado por una presencia guia!"), span_info("Empiezas a guiar a [C]."))
...
/obj/effect/proc_holder/spell/invoked/infestation5e/cast(list/targets, mob/living/user)
	if(isliving(targets[1]))
		var/mob/living/carbon/target = targets[1]
		target.visible_message(span_warning("[target] esta rodeado por una horda de alimañas pestilentes!"), span_notice("Rodeas a [target] en una horda de alimañas pestilentes!"))
...
/datum/status_effect/buff/infestation5e/on_apply()
	. = ..()
	var/mob/living/target = owner
	to_chat(owner, span_danger("De repente estoy rodeado por una horda de bichos!"))
	target.Jitter(20)
...
/obj/effect/proc_holder/spell/self/light5e/cast(list/targets, mob/user = usr)
	...
	if(user.dropItemToGround(user.get_active_held_item()))
		...
		user.visible_message(span_info("Un orbe de luz se condensa en la mano de [user]!"), span_info("Condensas un orbe de luz pura!"))
...
/obj/effect/proc_holder/spell/targeted/lightninglure5e/cast(list/targets, mob/user = usr)
	for(var/mob/living/carbon/C in targets)
		user.visible_message(span_warning("[C] esta conectado a [user] con un latigo de rayo!"), span_warning("Creas un enlace estatico con [C]."))
		...
		if (dist <= range)
			...
			user.visible_message(span_warning("El latigo de rayo se apaga!"), span_warning("[C] esta demasiado lejos!"))
...
/obj/effect/proc_holder/spell/invoked/magicstone5e/cast(list/targets, mob/living/user)
	if(istype(targets[1], /obj/item/natural/stone))
		...
		if (!S.magicstone)
			to_chat(user, "<span class='info'>[S] esta impregnado de energia magica!</span>")
			S.name = "magic "+S.name
			S.force *= 1.5 //ouchy
			...
			to_chat(user, span_warning("Esa piedra no puede ser mas magica!"))
			revert_cast()
	else
		to_chat(user, span_warning("No hay ni una piedra aqui!"))
		revert_cast()
...
/obj/effect/proc_holder/spell/invoked/mending5e/cast(list/targets, mob/living/user)
	if(istype(targets[1], /obj/item))
		...
		if(I.obj_integrity < I.max_integrity)	
			...
			user.visible_message(span_info("[I] brilla en una tenue luz reparadora."))
			if(I.obj_broken == TRUE)
				I.obj_broken = FALSE
		else
			user.visible_message(span_info("[I] parece estar en perfectas condiciones."))
			revert_cast()
	else
		to_chat(user, span_warning("No hay ningun objeto aqui!"))
		revert_cast()
...
/obj/effect/proc_holder/spell/invoked/mindsliver5e/cast(list/targets, mob/user)
	...
	for(var/mob/living/L in T.contents)
		...
		to_chat(L, "<span class='userdanger'>¡Energia psiquica es impulsada en mi craneo!</span>")
		...
		if(con)
			if(con.spillable)
				if(con.reagents.total_volume > 0)
					...
					user.visible_message(span_warning("[user] rocia el contenido del [held_item], creando una nube!"), span_warning("Rocias el contenido del [held_item], creando una nube!"))					
			...
			to_chat(user, "<span class='warning'>Necesito sostener un contenedor para utilizar esto!</span>")
			revert_cast()
	else
		to_chat(user, "<span class='warning'>No puedo encontrar un buen lugar para esto!</span>")
		revert_cast()

//==============================================
//	Primal Savagery
//==============================================
// TODO: TRADUCIR ESTO A LA VERGAAA
