...
/mob/dead/new_player/proc/LateChoices()
	...
	omegalist += list(GLOB.el_den_positions)
			...
			for(var/job in available_jobs)
				...
				if(job_datum)
					...
					if(job_datum in SSjob.prioritized_jobs)
						...
						dat += "<a class='job[command_bold]' href='byond://?src=[REF(src)];SelectedJob=[job_datum.title]'>[used_name] ([job_datum.current_positions])[job_datum.round_contrib_points ? "" : ""]</a>"
